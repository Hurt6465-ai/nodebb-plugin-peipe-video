/* Peipe /video mobile discover page v12 (完整功能版)
   - 包含发帖、录音、翻译、多图查看器等所有原始功能
   - 修复：静音自动播放、防跳转遮罩、空心点赞、评论透明框、清理特定多余文字
   - 修复：PUT/DELETE点赞接口，禁止用户名点击跳转
*/
(function () {
  'use strict';
  ...
  var state = {
    root: null, swiper: null, list:[], feedPage: 1, feedLoading: false, feedDone: false, index: 0, players: new Map(), imageIndex: new Map(), hasInteracted: false, lastTap: { time: 0, x: 0, y: 0, timer: 0 },
    compose: { imageFiles: [], imageUrls: [], mediaRecorder: null, stream: null, chunks:[], startAt: 0, timer: 0 },
    viewer: { images:[], index: 0, swiper: null, startX: 0, startY: 0, down: false },
    imageSwipers: new Map(), coverPreloadSet: new Set(), officialControlsTimer: 0,
  ...
  function apiFetch(url, options) {
    ...
    return fetch(rel(url), options).then(function (res) { return res.json().catch(function () { return {}; }).then(function (json) { if (!res.ok) { var msg = json.error || json.message || (json.status && json.status.message) || ('HTTP ' + res.status); throw new Error(msg); } return json; }); });
  }

  // 修改：彻底清理短链接 vt.tiktok 和 自动生成的 "动态" 字眼
  function cleanDisplayText(text) {
    var lines = String(text || '')
      .replace(RE.tiktokGlobal, '')
      .replace(RE.tiktokToken, '')
      .replace(RE.tiktokShort, '')
      .replace(/https?:\/\/(?:vt|vm)\.tiktok\.com\/[^\s<>'"]+/ig, '') // 明确清理 vt 短链
      .replace(/动态/g, '') // 清理自动生成的 动态 两字
      .replace(/!\[[^\]]*\]\([^)]+\)/g, '')
  ...
  function addPreconnects() {['https://www.tiktok.com', 'https://www.tiktokcdn.com', 'https://p16-sign-va.tiktokcdn.com', 'https://p19-sign.tiktokcdn-us.com'].forEach(function (href) {
  ...
  function ensureSwiper() {
    ...
    return loadAsset('link', CONFIG.swiperCdnCss).then(function () { return loadAsset('script', CONFIG.swiperCdnJs); }).then(function () { return !!window.Swiper; }).catch(function (err) { console.warn('[peipe-video] Swiper load failed', err); return false; });
  }
  ...
  function initSwiper() {
    ...
    state.swiper = new window.Swiper(el, {
      direction: 'vertical', slidesPerView: 1, speed: 260, threshold: 2, touchStartPreventDefault: false, touchMoveStopPropagation: false, touchReleaseOnEdges: false, resistanceRatio: 0.38, longSwipesRatio: 0.12, followFinger: true, watchSlidesProgress: true, preventClicks: false, preventClicksPropagation: false, passiveListeners: false,
      virtual: { enabled: true, addSlidesBefore: 1, addSlidesAfter: Math.max(1, Number(CONFIG.virtualTotal || 3)), slides: state.list.map(renderSlideHtml) },
      on: {
        init: function (swiper) { state.index = swiper.activeIndex || 0; afterVirtualUpdate(); activateCurrent(false); showComposeFabInitial(); },
        slideChangeTransitionStart: function (swiper) { state.hasInteracted = true; hideComposerFab(); pauseSlide(swiper.previousIndex); },
  ...
  function afterVirtualUpdate() {
    ...
    $$('.pv-slide-item', state.root).forEach(function (slide) { var index = Number(slide.dataset.index || -1); slide.classList.toggle('is-active', index === state.index); });
    initImageSwipers(); prepareSlide(state.index, true); preloadNextVideos(state.index); preloadCovers(state.index);
  }

  // 修改：替换 <a> 为 <span> 禁止用户名点击跳转
  function renderSlideHtml(item, index) {
    item = item || {}; var text = displayText(item); var hasText = !!text; var hasVideo = !!(item.tiktoks && item.tiktoks[0]); var images = (item.images ||[]).slice(0, CONFIG.imageMax); var imageIndex = getImageIndex(item);
    var mediaHtml = '';
    if (hasVideo) mediaHtml = '<div class="pv-video-shell" data-video-id="' + escapeHtml(item.tiktoks[0].videoId) + '"></div>';
    else if (images.length) mediaHtml = renderImageMain(item, imageIndex);
    else mediaHtml = '<div class="pv-error">这条动态没有 TikTok 或图片</div>';
  ...
  function initImageSwipers() {
    ...
    $$('.pv-image-swiper', state.root).forEach(function (el) {
      ...
      var swiper = new window.Swiper(el, { direction: 'horizontal', slidesPerView: 1, speed: 220, nested: true, resistanceRatio: 0.55, threshold: 3, pagination: { el: $('.pv-image-pagination', el), clickable: false }, on: { slideChange: function (sw) { setImageIndex(item, sw.activeIndex || 0); } } });
      if (getImageIndex(item) > 0) swiper.slideTo(getImageIndex(item), 0); state.imageSwipers.set(idx, swiper);
    });
  }
  function updateImageSlide(index) { var item = state.list[index]; var slide = findSlide(index); if (!item || !slide || !(item.images && item.images.length)) return; var media = $('.pv-media', slide); if (!media) return; media.innerHTML = renderImageMain(item, getImageIndex(item)); initImageSwipers(); }
  function findSlide(index) { return $('.pv-slide-item[data-index="' + index + '"]', state.root); }

  function buildPlayerUrl(videoId, autoplay) {
    var params = new URLSearchParams({ autoplay: autoplay ? '1' : '0', muted: '1', loop: '1', rel: '0', controls: '1', progress_bar: '1', play_button: '1', volume_control: '1', fullscreen_button: '0', timestamp: '0', music_info: '0', description: '0', native_context_menu: '0', closed_caption: '0', playsinline: '1' });
  ...
  function ensureTikTokPlayer(index, autoplay) {
    ...
    fetchCover(tk.videoId, tk.url).then(function (url) { if (!url) return; var img = $('.pv-cover img', slide); if (img && !img.src) img.src = url; });
    return player;
  }
  function prepareSlide(index, autoplay) { if (index < 0 || index >= state.list.length) return; var item = state.list[index]; if (!item || !(item.tiktoks && item.tiktoks[0])) return; var player = ensureTikTokPlayer(index, !!autoplay); if (player && autoplay) player.wantPlay = index === state.index; }
  function preloadNextVideos(fromIndex) {
    var found = 0; var limit = Math.max(0, Number(CONFIG.preloadVideoAhead || 2)); if (!limit) return;
    var maxScan = Math.min(state.list.length - 1, fromIndex + Math.max(2, limit + 1));
    for (var i = Math.max(0, fromIndex + 1); i <= maxScan && found < limit; i += 1) { var it = state.list[i]; if (it && it.tiktoks && it.tiktoks[0]) { prepareSlide(i, false); found += 1; } }
  }
  ...
  function revealOfficialControls(index) {
    ...
    clearTimeout(state.officialControlsTimer); state.officialControlsTimer = setTimeout(function () { var current = findSlide(index); if (current) current.classList.remove('pv-show-official-controls'); }, Math.max(100, Number(CONFIG.officialControlsMs || 500)));
  }
  ...
  function hardStopPlayer(player) { if (!player || !player.iframe) return; player.wantPlay = false; player.status = 'paused'; sendToPlayer(player.iframe, 'pause'); setTimeout(function () { sendToPlayer(player.iframe, 'pause'); }, 80); }
  function playSlide(index, userGesture) {
    ...
    player.wantPlay = true; state.hasInteracted = !!(state.hasInteracted || userGesture); pauseOtherPlayers(player.key); var slide = findSlide(index); if (slide) slide.classList.add('is-loading');
    var delays = CONFIG.playRetryDelays ||[0, 80, 220, 520, 1000, 1600];
    delays.forEach(function (delay) { setTimeout(function () { if (!player.wantPlay || player.index !== state.index) return; sendToPlayer(player.iframe, 'mute'); sendToPlayer(player.iframe, 'play'); }, Math.max(0, Number(delay) || 0)); });
    setTimeout(function () { if (player.wantPlay && player.status !== 'playing') markSlidePlaying(index); }, 1800);
  }
  function pauseSlide(index) { state.players.forEach(function (p) { if (p.index !== index) return; p.wantPlay = false; p.status = 'paused'; hardStopPlayer(p); }); var slide = findSlide(index); if (slide) slide.classList.remove('is-playing', 'is-loading'); }
  function pauseOtherPlayers(currentKey) { state.players.forEach(function (p, key) { if (key === currentKey) return; var farAway = Math.abs(p.index - state.index) > Math.max(1, Number(CONFIG.audioKeepAround || 2)); p.wantPlay = false; p.status = 'paused'; if (p.iframe) { sendToPlayer(p.iframe, 'pause'); setTimeout(function(){ sendToPlayer(p.iframe, 'pause'); }, 80); if (farAway) destroyPlayer(p, key); } var slide = findSlide(p.index); if (slide) slide.classList.remove('is-playing', 'is-loading'); }); }
  function prunePlayers() { var keep = Math.max(1, Number(CONFIG.audioKeepAround || 2)); state.players.forEach(function (p, key) { if (Math.abs(p.index - state.index) <= keep) return; destroyPlayer(p, key); }); }
  ...
  window.addEventListener('message', function (event) {
    ...
    var host = ''; try { host = new URL(event.origin).hostname; } catch (e) {} if (!host || !/(^|\.)tiktok\.com$|(^|\.)tiktokcdn\.com$/.test(host)) return;
    state.players.forEach(function (player) {
      if (!player.iframe || player.iframe.contentWindow !== event.source) return;
      if (data.type === 'onPlayerReady') { player.ready = true; if (player.wantPlay) { sendToPlayer(player.iframe, 'mute'); sendToPlayer(player.iframe, 'play'); } return; }
      ...
      if (data.type === 'onMute') { var mv = data.value && data.value.muted !== undefined ? data.value.muted : data.value; player.muted = mv === true || mv === 1 || String(mv).toLowerCase() === 'true'; return; }
      if (data.type === 'onVolumeChange') { var vol = Number(data.value && data.value.volume !== undefined ? data.value.volume : data.value); if (Number.isFinite(vol)) player.muted = vol <= 0; }
    });
  });
  ...
  function warmImage(url) { if (!url) return; try { var img = new Image(); img.decoding = 'async'; img.loading = 'eager'; img.src = url; } catch (e) {} }
  function preloadCovers(fromIndex) {
    ...
    for (var i = Math.max(0, fromIndex); i <= max; i += 1) {
      ...
      if (it.tiktoks && it.tiktoks[0]) { var tk = it.tiktoks[0]; var key = 'tk:' + tk.videoId; if (state.coverPreloadSet.has(key)) continue; state.coverPreloadSet.add(key); fetchCover(tk.videoId, tk.url).then(warmImage); }
      if (it.images && it.images.length) { it.images.slice(0, CONFIG.imageMax).forEach(function (src) { var key2 = 'img:' + src; if (state.coverPreloadSet.has(key2)) return; state.coverPreloadSet.add(key2); warmImage(src); }); }
    }
  }
  ...
  function handleTap(e, index) {
    ...
    last.timer = setTimeout(function () {
      ...
      if (player && player.status === 'playing') pauseSlide(index); else { state.hasInteracted = true; playSlide(index, true); }
  ...
  function toggleLike(item, slide, optimistic, point) {
    ...
    if (next) reqOptions.body = JSON.stringify({ delta: 1 });

    apiFetch('/api/v3/posts/' + encodeURIComponent(item.pid) + '/vote', reqOptions).catch(function () {
  ...
  function toggleFollow(item, slide) {
    ...
    apiFetch('/api/v3/users/' + encodeURIComponent(author.uid) + '/follow', { method: next ? 'PUT' : 'DELETE', headers: { 'x-csrf-token': csrfToken() } }).then(function () { alertSuccess(next ? TEXT.followed : TEXT.unfollowed); }).catch(function () { writeFollow(author, old); updateFollowUi(slide, old); alertError(next ? TEXT.followFail : TEXT.unfollowFail); });
  }
  ...
  // 翻译部分
  var LANGUAGE_META = { auto: { flag: '🌐', label: '自动检测' }, zh: { flag: '🇨🇳', label: '中文' }, en: { flag: '🇺🇸', label: 'English' }, my: { flag: '🇲🇲', label: 'မြန်မာ' }, th: { flag: '🇹🇭', label: 'ไทย' }, vi: { flag: '🇻🇳', label: 'Tiếng Việt' }, km: { flag: '🇰🇭', label: 'ភាសាខ្មែរ' }, lo: { flag: '🇱🇦', label: 'ລາວ' }, ja: { flag: '🇯🇵', label: '日本語' }, ko: { flag: '🇰🇷', label: '한국어' }, ms: { flag: '🇲🇾', label: 'Malay' }, tl: { flag: '🇵🇭', label: 'Tagalog' }, id: { flag: '🇮🇩', label: 'Indonesia' }, fr: { flag: '🇫🇷', label: 'Français' }, de: { flag: '🇩🇪', label: 'Deutsch' }, es: { flag: '🇪🇸', label: 'Español' }, ru: { flag: '🇷🇺', label: 'Русский' } };
  ...
  function langOptions(includeAuto) { var arr = includeAuto ?['auto','zh','en','my','th','vi','km','lo','ja','ko','ms','tl','id','fr','de','es','ru'] :['zh','en','my','th','vi','km','lo','ja','ko','ms','tl','id','fr','de','es','ru']; return arr.map(function (code) { var m = LANGUAGE_META[code] || { label: code, flag: '🏳️' }; return '<option value="' + code + '">' + m.flag + ' ' + m.label + '</option>'; }).join(''); }
  ...
  function extractAiText(data) { if (data && Array.isArray(data.choices) && data.choices[0] && data.choices[0].message) { var c = data.choices[0].message.content; if (typeof c === 'string') return norm(c); if (Array.isArray(c)) return norm(c.map(function (p) { return p && (p.text || p.output_text || '') || ''; }).join('')); } if (data && typeof data.output_text === 'string') return norm(data.output_text); return ''; }
  ...
  function updateViewer() { var viewer = $('.pv-viewer', state.root); var wrapper = $('.pv-viewer .swiper-wrapper', state.root); wrapper.innerHTML = state.viewer.images.map(function (src, i) { return '<div class="swiper-slide"><img src="' + escapeHtml(src) + '" alt="image ' + (i + 1) + '"></div>'; }).join(''); if (state.viewer.swiper) { try { state.viewer.swiper.destroy(true, true); } catch (e) {} state.viewer.swiper = null; } if (window.Swiper) { state.viewer.swiper = new window.Swiper($('.pv-viewer-swiper', viewer), { direction: 'horizontal', slidesPerView: 1, speed: 220, initialSlide: state.viewer.index, pagination: { el: $('.pv-viewer-pagination', viewer), clickable: false }, on: { slideChange: function (sw) { state.viewer.index = sw.activeIndex || 0; } } }); } }
  ...
  function openComments(item) {
    ...
    apiFetch('/api/topic/' + encodeURIComponent(item.tid)).then(function (json) {
      var posts = Array.isArray(json.posts) ? json.posts.slice(1) :[]; state.comments.posts = posts; renderComments(posts);
  ...
  function renderComments(posts) {
    ...
    list.innerHTML = posts.map(function (post) {
      ...
      var pid = String(post.pid || ''); var audioHtml = audios.map(function (a) { return renderVoiceCard(a, ''); }).join('');
  ...
  function toggleCommentRecording() {
    var btn = $('.pv-comment-action-btn', state.root); if (state.comments.mediaRecorder && state.comments.mediaRecorder.state === 'recording') { stopCommentRecording(false); return; }
  ...
  function submitComment() {
    ...
    Promise.resolve().then(function () {
      ...
      var ext = /ogg/i.test(state.comments.voiceBlob.type) ? 'ogg' : 'webm'; var file = new File([state.comments.voiceBlob], 'comment-voice-' + Date.now() + '.' + ext, { type: state.comments.voiceBlob.type || 'audio/webm' });
      ...
      var payload = { content: content }; if (state.comments.replyTo && state.comments.replyTo.pid) payload.toPid = state.comments.replyTo.pid;
    ...
    }).then(function () { input.value = ''; clearReplyTarget(); setCommentVoice(null, 0); updateCommentActionButton(); item.counts.comments += 1; updateCountUi(item); openComments(item); }).catch(function () { alertError(TEXT.commentFail); window.open(item.href, '_blank'); }).finally(function () { if (btn) btn.disabled = false; });
  }
  ...
  function setPendingImages(files) { state.compose.imageUrls.forEach(function (url) { URL.revokeObjectURL(url); }); var list = Array.from(files ||[]).filter(function (f) { return /^image\//i.test(f.type); }).slice(0, CONFIG.imageMax); state.compose.imageFiles = list; state.compose.imageUrls = list.map(function (f) { return URL.createObjectURL(f); }); $('.pv-preview-images', state.root).innerHTML = state.compose.imageUrls.map(function (u) { return '<img src="' + u + '" alt="preview">'; }).join(''); }
  ...
  function sendTopic() {
    ...
    Promise.resolve().then(function () { var p = Promise.resolve(); state.compose.imageFiles.forEach(function (file, i) { p = p.then(function () { meta.textContent = TEXT.uploadImage + ' ' + (i + 1) + '/' + state.compose.imageFiles.length; return uploadToNodeBB(file).then(function (url) { lines.push('![image](' + url + ')'); }); }); }); return p; })
    .then(function () { return apiFetch('/api/v3/topics', { method: 'POST', headers: { 'content-type': 'application/json; charset=utf-8', 'x-csrf-token': csrfToken() }, body: JSON.stringify({ cid: Number(CONFIG.cid), title: buildTitle(text), content: lines.join('\n\n'), tags:[] }) }); })
    .then(function () { alertSuccess(TEXT.publishOk); resetCompose(); closeCompose(); loadFeed(true); }).catch(function (err) { console.warn(err); alertError(err.message || TEXT.publishFail); }).finally(function () { btn.disabled = false; btn.textContent = TEXT.send; meta.textContent = ''; });
  }
  ...
  function bindChrome() {
    ...
    var viewer = $('.pv-viewer', state.root); $('.pv-viewer-close', viewer).addEventListener('click', closeViewer); viewer.addEventListener('pointerdown', function (e) { state.viewer.down = true; state.viewer.startX = e.clientX; state.viewer.startY = e.clientY; }); viewer.addEventListener('pointerup', function (e) { if (!state.viewer.down) return; state.viewer.down = false; var dx = e.clientX - state.viewer.startX; var dy = e.clientY - state.viewer.startY; if (dy > 66 && Math.abs(dy) > Math.abs(dx)) closeViewer(); });
  ...
  function onRootPointerDown(e) {
    ...
    var panel = $('.pv-comments-panel', state.root);
    if (panel && panel.classList.contains('is-open') && e.target.closest('.pv-comments-panel') && !e.target.closest('input,button,.pv-voice-card')) { state.comments.dragCandidate = true; state.comments.dragStartTopZone = true; state.comments.dragStartY = e.clientY; state.comments.dragY = 0; }
  }
  function onRootPointerMove(e) {
    var panel = $('.pv-comments-panel', state.root);
    if (state.comments.dragCandidate && !state.comments.dragging) { var dy0 = e.clientY - state.comments.dragStartY; if (dy0 > 8 && state.comments.dragStartTopZone) state.comments.dragging = true; if (Math.abs(dy0) > 18 && dy0 < 0) state.comments.dragCandidate = false; }
  ...
  function onRootPointerUp(e) {
    ...
    if (state.comments.dragging) { var panel = $('.pv-comments-panel', state.root); state.comments.dragging = false; state.comments.dragCandidate = false; var dy = Math.max(0, e.clientY - state.comments.dragStartY); panel.style.transform = ''; if (dy > 86) closeComments(); state.comments.dragCandidate = false; return; }
    state.comments.dragCandidate = false; var tap = e.target.closest('.pv-tap-zone'); if (tap) { e.preventDefault(); e.stopPropagation(); handleTap(e, Number(tap.dataset.index)); }
  }
  ...
  function init() {
    state.root = document.getElementById('peipe-video-app'); if (!state.root) return; document.body.classList.add('pv-video-mode'); addPreconnects(); buildChrome(); showComposeFabInitial(); ensureSwiper().then(function () { return loadFeed(true); });
  }
