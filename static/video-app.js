/* Peipe /video mobile discover page v12
   ...
   - Muted autoplay via postMessage (muted=0 so user CAN unmute via official TikTok button)
   - sandbox="allow-scripts allow-same-origin allow-popups" on iframe to prevent redirect to download page
   - Cover preload 6 ahead, video iframe preload 1-2 ahead, instant switch
   - Input area fully transparent (no background box)
   - Comment panel adaptive height based on content
   - Compose panel has translate button
   - Hide "https://vt." short links and auto-generated "动态" text
   - Only avatar click navigates to profile; username and screen tap do NOT navigate
   - Hollow heart when not liked, solid red when liked
   - Correct PUT/DELETE vote API for like/unlike
   - Global sound state: muted by default, user toggles via top-right button
   - iframe reuse + lazy destroy for smooth scrolling
*/
(function () {
  'use strict';
  ...
  var state = {
    ...
    imageIndex: new Map(),
    hasInteracted: false,
    // Global muted state: starts muted, user toggles via sound button
    globalMuted: true,
  ...
  /* ── Text cleaning: hide vt. links, "动态", TikTok URLs ── */
  ...
  function isAutoText(text) {
    ...
    return !clean ||
      /^(?:新动态|动态|图片分享|图片动态|语音消息|语音动态|voice message|audio message|image|photo|picture)(?:\s*:?\s*\d{1,2}:\d{2}(?::\d{2})?)?$/i.test(clean) ||
      /^https?:\/\/vt\./i.test(clean);
  }
  ...
  function renderSlideHtml(item, index) {
    ...
    // Username is a plain span, NOT a link (only avatar links to profile)
    return '' +
      '<section class="pv-slide-item ' + (hasVideo ? 'is-video' : 'is-image') + '" data-index="' + index + '" data-tid="' + escapeHtml(item.tid || '') + '">' +
        ...
        /* Sound toggle button */
        (hasVideo ? '<button type="button" class="pv-sound-btn" data-index="' + index + '" aria-label="声音">' + iconSoundOff() + '</button>' : '') +
        '<div class="pv-toolbar">' +
          '<div class="pv-avatar-wrap"><a class="pv-avatar-link" href="' + authorHref(author) + '">' + avatarHtml + '</a>' +
        ...
        '</div>' +
        '<div class="pv-desc">' +
          /* Username is a span, not an <a> — no navigation on click */
  ...
  function iconMic() { return '<svg class="pv-mic-svg" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 14a3 3 0 0 0 3-3V6a3 3 0 0 0-6 0v5a3 3 0 0 0 3 3z" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"></path><path d="M5 11a7 7 0 0 0 14 0M12 18v3m-4 0h8" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"></path></svg>'; }
  function iconSend() { return '<svg class="pv-send-svg" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 12h14M13 6l6 6-6 6" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"></path></svg>'; }
  ...
  function iconSoundOn() { return '<svg viewBox="0 0 24 24" fill="white"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>'; }
  function iconSoundOff() { return '<svg viewBox="0 0 24 24" fill="white"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/></svg>'; }
  ...
  function buildPlayerUrl(videoId, autoplay) {
    // muted=0 allows user to unmute via official TikTok volume control
    // We control mute/unmute via postMessage
  ...
  function ensureTikTokPlayer(index, autoplay) {
    ...
    // sandbox prevents iframe from navigating top frame (no redirect to download page)
    iframe.sandbox = 'allow-scripts allow-same-origin allow-popups allow-popups-to-user-action allow-presentation';
  ...
  function preloadNextVideos(fromIndex) {
    var found = 0;
    var limit = Math.max(0, Number(CONFIG.preloadVideoAhead || 1));
  ...
  function toggleGlobalMute() {
    ...
    if (item && item.tiktoks && item.tiktoks[0]) {
      var key = playerKey(state.index, item.tiktoks[0].videoId);
      var player = state.players.get(key);
      if (player && player.iframe) {
        ...
        player.muted = state.globalMuted;
      }
    }
    updateSoundButtonIcon();
  }
  ...
  function updateSoundButtonIcon() {
    ...
    $$('.pv-sound-btn', state.root).forEach(function (btn) {
      btn.innerHTML = state.globalMuted ? iconSoundOff() : iconSoundOn();
    });
  }
  ...
  function playSlide(index, userGesture) {
    ...
    delays.forEach(function (delay) {
      setTimeout(function () {
        ...
        // Always mute first for autoplay, then apply global state
        sendToPlayer(player.iframe, 'mute');
        sendToPlayer(player.iframe, 'play');
        // After play starts, apply the user's desired mute state
        setTimeout(function () {
          if (player.wantPlay) applyMuteState(player);
        }, 100);
      }, Math.max(0, Number(delay) || 0));
    });
    // Fallback: mark playing after timeout
    ...
    state.players.forEach(function (player) {
      ...
      if (data.type === 'onPlayerReady') {
        ...
        if (player.wantPlay) {
          sendToPlayer(player.iframe, 'mute');
          sendToPlayer(player.iframe, 'play');
          setTimeout(function () { if (player.wantPlay) applyMuteState(player); }, 150);
        }
        return;
      }
      if (data.type === 'onStateChange') {
        ...
        if (value === 1 || word === 'playing') {
          ...
          // Apply correct mute state once playing
          setTimeout(function () { applyMuteState(player); }, 50);
        } else if (value === 2 || value === 0 || word === 'paused' || word === 'ended') {
          player.status = 'paused';
          var s = findSlide(player.index); if (s) s.classList.remove('is-playing', 'is-loading');
  ...
  function warmImage(url) {
    if (!url) return;
    try { var img = new Image(); img.decoding = 'async'; img.loading = 'eager'; img.src = url; } catch (e) {}
  }
  function preloadCovers(fromIndex) {
    var ahead = Math.max(0, Number(CONFIG.coverPreloadAhead || 6));
    ...
    return new Promise(function (resolve) {
      ...
      img.onload = function () {
        ...
        canvas.toBlob(function (blob) {
          ...
          var name = String(file.name || ('image-' + Date.now())).replace(/\.[^.]+$/, '') + '.jpg';
  ...
  function buildTitle(text) { var clean = norm(String(text || '').replace(RE.tiktokGlobal, '').replace(RE.tiktokShort, '').replace(RE.vtLink, '').replace(/!\[[^\]]*\]\([^)]+\)/g, '').replace(/\[[^\]]*\]\([^)]+\)/g, '')); return clean ? clean.slice(0, 80) : '动态'; }
  ...
  /* Compose: translate textarea content */
  function translateComposeText() {
    var textarea = $('.pv-compose-panel textarea', state.root);
    var text = norm(textarea.value);
    if (!text) return;
    var meta = $('.pv-compose-panel .pv-meta', state.root);
    meta.textContent = TEXT.translating;
    translateText(text).then(function (out) {
      if (out) { textarea.value = out; meta.textContent = ''; }
      else { meta.textContent = TEXT.translateFail; }
    }).catch(function () { meta.textContent = TEXT.translateFail; });
  }
  ...
  function bindChrome() {
    ...
    // Compose translate button
    $('.pv-compose-translate-btn', state.root).addEventListener('click', translateComposeText);
    bindLongPress($('.pv-compose-translate-btn', state.root), openTranslateSettings);
  ...
  function onRootClick(e) {
    var btn;
    // Sound toggle button
    if ((btn = e.target.closest('.pv-sound-btn'))) { e.preventDefault(); e.stopPropagation(); toggleGlobalMute(); return; }
    ...
    // Block username click (it's a span now, but just in case)
    if (e.target.closest('.pv-username')) { e.preventDefault(); e.stopPropagation(); return; }
  }
