...
(function () {
  ...
  function safeJsonGet(key, fb) { try { var raw = localStorage.getItem(key); return raw ? JSON.parse(raw) : fb; } catch (e) { return fb; } }
  function safeJsonSet(key, val) { try { localStorage.setItem(key, JSON.stringify(val)); } catch (e) {} }
  ...
  // 文本清洗：去掉 TikTok 短链 / 长链 / vt. 链接 / "动态" 占位
  function cleanDisplayText(text) {
    ...
    raw = raw
      ...
      .replace(RE.bareVtLine, '')
      .replace(/!\[[^\]]*\]\([^)]+\)/g, '')
      .replace(/\[\s*(?:语音消息|语音动态|voice\s*message|audio\s*message)[^\]]*\]\([^)]+\)/ig, '');
    var lines = raw.split(/[\r\n]+/).map(function (l) { return norm(l); }).filter(function (l) {
      if (!l) return false;
      if (/^https?:\/\/vt\./i.test(l)) return false;
      if (isAutoText(l)) return false;
      return true;
    });
    return lines.join('\n');
  }
  function isAutoText(text) {
    ...
    return !clean || /^(?:新动态|动态|图片分享|图片动态|语音消息|语音动态|voice message|audio message|image|photo|picture)(?:\s*:??\s*\d{1,2}:\d{2}(?::\d{2})?)?$/i.test(clean);
  }
  ...
  function isOwnAuthor(author) {
    ...
    return String(me.uid || '') === String(author.uid || '') ||
      String(me.userslug || '').toLowerCase() === String(author.userslug || '').toLowerCase();
  }
  ...
  function addPreconnects() {
    ['https://www.tiktok.com', 'https://www.tiktokcdn.com', 'https://p16-sign-va.tiktokcdn.com', 'https://p19-sign.tiktokcdn-us.com'].forEach(function (href) {
      ...
      var dns = document.createElement('link'); dns.rel = 'dns-prefetch'; dns.href = href;
  ...
  function loadFeed(refresh) {
    ...
    if (refresh) {
      state.feedPage = 1; state.feedDone = false; state.list = [];
      state.index = 0; state.players.clear();
    ...
    return apiFetch('/api/v3/plugins/peipe-video/feed?page=' + page + '&pageSize=' + CONFIG.pageSize)
      .then(function (json) {
        ...
        var items = (payload.items || []).map(function (item) {
          ...
          return (item.tiktoks && item.tiktoks.length) || (item.images && item.images.length);
        });
      ...
      .catch(function (err) {
        console.error('[peipe-video] feed failed', err);
    ...
    state.swiper = new window.Swiper(el, {
      ...
      on: {
        ...
        slideChange: function (swiper) {
          ...
          var curKey = cur && cur.tiktoks && cur.tiktoks[0] ? playerKey(state.index, cur.tiktoks[0].videoId) : '';
          pauseOtherPlayers(curKey);
          activateCurrent(false);
        },
  ...
  function renderSlideHtml(item, index) {
    ...
    var avatarImg = avatar
      ? '<img class="pv-avatar" src="' + escapeHtml(avatar) + '" alt="avatar">'
    ...
    return '' +
      '<section class="pv-slide-item ' + (hasVideo ? 'is-video' : 'is-image') + '" data-index="' + index + '" data-tid="' + escapeHtml(item.tid || '') + '">' +
        ...
        // 双击点赞 / 单击播放热区（仅视频帖；底部留空给官方控件）
        (hasVideo ? '<div class="pv-tap-zone" data-index="' + index + '"></div>' : '') +

        '<div class="pv-toolbar">' +
          '<div class="pv-avatar-wrap">' +
            // 仅头像可点跳转
            '<a class="pv-avatar-link" href="' + authorHref(author) + '" data-index="' + index + '">' + avatarImg + '</a>' +
            (author.uid && !isOwnAuthor(author)
              ? '<button type="button" class="pv-follow-plus ' + (following ? 'is-following' : '') + '" data-index="' + index + '">' + (following ? '✓' : '+') + '</button>'
              : '') +
          '</div>' +
          '<button type="button" class="pv-action pv-like ' + (liked ? 'is-active' : '') + '" data-index="' + index + '">' +
            '<span class="pv-action-icon">' + iconHeart() + '</span>' +
            '<span>' + formatCount(item.counts.likes) + '</span>' +
          '</button>' +
          '<button type="button" class="pv-action pv-comment-btn" data-index="' + index + '">' +
            '<span class="pv-action-icon">' + iconComment() + '</span>' +
            '<span>' + formatCount(item.counts.comments) + '</span>' +
          '</button>' +
          (images.length && hasVideo
            ? '<button type="button" class="pv-action pv-album-btn" data-index="' + index + '"><span class="pv-action-icon">' + iconPhoto() + '</span><span>' + images.length + '</span></button>'
            : '') +
        '</div>' +
        ...
        '<div class="pv-desc">' +
          ...
          (hasText
            ? '<div class="pv-text-row" data-index="' + index + '"><span class="pv-text-main">' + escapeHtml(text) + '</span> <button type="button" class="pv-translate-btn" data-index="' + index + '" aria-label="' + TEXT.translate + '" title="' + TEXT.translate + '">' + iconTranslate() + '</button></div><div class="pv-translated"></div>'
            : '') +
        '</div>' +
      '</section>';
  }
  ...
  function iconHeart() {
    return '<svg class="pv-heart-svg" viewBox="0 0 48 48" aria-hidden="true"><path d="M24 41s-2.2-1.3-5.2-3.4C10.2 31.4 5 25.8 5 18.6 5 12.7 9.5 8 15.2 8c3.5 0 6.6 1.8 8.8 4.7C26.2 9.8 29.3 8 32.8 8 38.5 8 43 12.7 43 18.6c0 7.2-5.2 12.8-13.8 19C26.2 39.7 24 41 24 41z"></path></svg>';
  }
  function iconComment() {
    return '<svg class="pv-comment-svg" viewBox="0 0 48 48" aria-hidden="true"><path d="M24 7.5c-9.8 0-17.5 6.8-17.5 15.2 0 5.4 3.2 10.2 8 12.9l-.8 5.2 6.1-3.4c1.4.3 2.8.4 4.2.4 9.8 0 17.5-6.8 17.5-15.1S33.8 7.5 24 7.5z"></path><circle cx="17.4" cy="23.3" r="2.1"></circle><circle cx="24" cy="23.3" r="2.1"></circle><circle cx="30.6" cy="23.3" r="2.1"></circle></svg>';
  }
  function iconTranslate() { return '<i class="fa-solid fa-language" aria-hidden="true"></i>'; }
  function iconMic() {
    return '<svg class="pv-mic-svg" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" width="20" height="20"><path d="M12 14a3 3 0 0 0 3-3V6a3 3 0 0 0-6 0v5a3 3 0 0 0 3 3z"></path><path d="M5 11a7 7 0 0 0 14 0M12 18v3m-4 0h8"></path></svg>';
  }
  function iconSend() {
    return '<svg class="pv-send-svg" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" width="20" height="20"><path d="M4 12h14M13 6l6 6-6 6"></path></svg>';
  }
  function iconPhoto() {
    return '<svg viewBox="0 0 48 48" aria-hidden="true" fill="currentColor"><path d="M10 10h28a4 4 0 0 1 4 4v20a4 4 0 0 1-4 4H10a4 4 0 0 1-4-4V14a4 4 0 0 1 4-4zm5 23h18l-6-8-4 5-3-4-5 7zm2-13a4 4 0 1 0 0-8 4 4 0 0 0 0 8z"></path></svg>';
  }
  ...
  function pauseOtherPlayers(currentKey) {
    state.players.forEach(function (p, key) {
      ...
      var farAway = Math.abs(p.index - state.index) > Math.max(1, Number(CONFIG.audioKeepAround || 1));
  ...
  function prunePlayers() {
    var keep = Math.max(1, Number(CONFIG.audioKeepAround || 1));
    state.players.forEach(function (p, key) {
  ...
  /* ======================
     点赞 / 取消点赞
     PUT/DELETE /api/v3/posts/{pid}/vote
     ====================== */
  ...
  function updateLikeUi(slide, item) {
    ...
    if (count) count.textContent = formatCount(item.counts.likes);
  }
  ...
  function normalizeLang(code, fallback) {
    code = norm(code).toLowerCase().replace(/_/g, '-');
    if (!code) return fallback || 'auto';
    var short = code.split('-')[0];
    return LANGUAGE_META[code] ? code : (LANGUAGE_META[short] ? short : (fallback || code));
  }
  function langOptions(includeAuto) {
    var arr = includeAuto
      ? ['auto','zh','en','my','th','vi','km','lo','ja','ko','ms','tl','id','fr','de','es','ru']
      : ['zh','en','my','th','vi','km','lo','ja','ko','ms','tl','id','fr','de','es','ru'];
    return arr.map(function (code) {
      var m = LANGUAGE_META[code] || { label: code, flag: '🏳️' };
      return '<option value="' + code + '">' + m.flag + ' ' + m.label + '</option>';
    }).join('');
  }
  ...
  function translateCacheKey(text) {
    ...
    return 'pv-tr:' + s.provider + ':' + (s.aiModel || 'google') + ':' + s.sourceLang + ':' + s.targetLang + ':' + encodeURIComponent(norm(text)).slice(0, 240);
  }
  ...
  function translateViaAI(text, settings) {
    ...
    var prompt = String(settings.aiPrompt || DEFAULT_AI_PROMPT)
    ...
    return fetch(normalizeAiEndpoint(settings.aiEndpoint), {
      ...
      return res.json().catch(function () { return {}; }).then(function (json) {
        if (!res.ok) throw new Error(json.error && json.error.message || 'AI translate failed');
        return json;
      });
    }).then(function (json) {
  ...
  function translateViaGoogle(text, settings) {
    var url = 'https://translate.googleapis.com/translate_a/single?' + new URLSearchParams({
      client: 'gtx',
      sl: settings.sourceLang || 'auto',
      tl: settings.targetLang || 'zh',
      dt: 't',
      q: text
    }).toString();
  ...
  function translateElementText(text, target) {
    ...
    target.textContent = TEXT.translating;
    return translateText(text).then(function (out) {
      target.textContent = out || '';
      if (!out) target.classList.remove('is-show');
    }).catch(function () { target.textContent = TEXT.translateFail; });
  }
  function openTranslateSettings() {
    ...
    $$('.pv-provider-tab', panel).forEach(function (t) {
      t.classList.toggle('is-active', (t.dataset.provider || 'google') === (s.provider || 'google'));
    });
  ...
  function updateViewer() {
    ...
    if (window.Swiper) {
      state.viewer.swiper = new window.Swiper($('.pv-viewer-swiper', viewer), {
        ...
        slidesPerView: 1,
        speed: 220,
        initialSlide: state.viewer.index,
  ...
  function isAudioHref(href) {
    return RE.audioExt.test(String(href || '').split('?')[0]) ||
      /[?&](?:haa8dur|dur|duration)=/i.test(String(href || ''));
  }
  function parseDurationFromUrl(url) {
    try {
      var u = new URL(String(url || ''), location.origin);
      var raw = u.searchParams.get('haa8dur') || u.searchParams.get('dur') || u.searchParams.get('duration');
      return Math.max(0, Number(raw || 0) || 0);
    } catch (e) {
      var m = String(url || '').match(/[?&](?:haa8dur|dur|duration)=(\d+(?:\.\d+)?)/i);
      return m ? Number(m[1]) || 0 : 0;
    }
  }
  function extractAudiosFromContent(content) {
    var out = []; var raw = String(content || '');
    raw.replace(/<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/ig, function (m, href, label) {
      if (isAudioHref(href) && !out.some(function (a) { return a.url === href; })) {
        out.push({ url: href, label: norm(label.replace(/<[^>]*>/g, '')) || TEXT.voiceMsg });
      }
      return m;
    });
    raw.replace(/\[([^\]]*)\]\(([^)]+)\)/g, function (m, label, href) {
      if (isAudioHref(href) && !out.some(function (a) { return a.url === href; })) {
        out.push({ url: href, label: norm(label) || TEXT.voiceMsg });
      }
      return m;
    });
    return out;
  ...
  function voiceBars() {
    return [9,14,19,11,22,16,12,20,10,17,13,18].map(function (h) {
      return '<i style="height:' + h + 'px"></i>';
    }).join('');
  }
  function renderVoiceCard(audio, cls) {
    var dur = parseDurationFromUrl(audio.url);
    return '<button type="button" class="pv-voice-card ' + (cls || '') + '" data-src="' + escapeHtml(audio.url) + '" data-duration="' + dur + '">' +
      '<span class="pv-voice-play">▶</span>' +
      '<span class="pv-voice-wave">' + voiceBars() + '</span>' +
      '<span class="pv-voice-time">' + formatDuration(dur) + '</span>' +
      '</button>';
  }
  function toggleVoiceCard(card) {
    ...
    if (!src) return;
    var audio = card._pvAudio || new Audio(src);
    card._pvAudio = audio;
    audio.preload = 'metadata';

    $$('.pv-voice-card.playing', state.root).forEach(function (node) {
      if (node !== card && node._pvAudio) {
        node._pvAudio.pause();
        node.classList.remove('playing');
        $('.pv-voice-play', node).textContent = '▶';
      }
    });
    audio.onloadedmetadata = function () {
      if (audio.duration && isFinite(audio.duration)) $('.pv-voice-time', card).textContent = formatDuration(audio.duration);
    };
    audio.ontimeupdate = function () {
      if (!audio.paused) $('.pv-voice-time', card).textContent = formatDuration(audio.currentTime);
    };
    audio.onended = function () {
      card.classList.remove('playing');
      $('.pv-voice-play', card).textContent = '▶';
      audio.currentTime = 0;
    };
    if (audio.paused) {
  ...
  function openComments(item) {
    ...
    apiFetch('/api/topic/' + encodeURIComponent(item.tid)).then(function (json) {
      ...
      $('.pv-comments-list', panel).innerHTML = '<div class="pv-meta">加载失败，<a href="' + item.href + '">' + TEXT.openTopic + '</a></div>';
    });
  }
  function closeComments() {
    stopCommentRecording(true);
  ...
  function renderComments(posts) {
    ...
    if (!posts.length) {
      list.innerHTML = '<div class="pv-meta pv-empty-comments">' + TEXT.noComments + '</div>';
      return;
    }
    list.innerHTML = posts.map(function (post) {
      ...
      var avatar = user.picture
        ? '<img src="' + escapeHtml(user.picture) + '" alt="avatar">'
        : '<img alt="avatar">';
      ...
      return '<div class="pv-comment" data-pid="' + escapeHtml(pid) + '" data-username="' + escapeHtml(user.username || '用户') + '">' +
        ...
        '<div class="pv-comment-body">' +
          ...
          (text ? '<div class="pv-comment-text">' + escapeHtml(text) + '</div>' : '') +
          audioHtml +
          ...
          '<div class="pv-comment-actions">' +
            ...
            '<button type="button" class="pv-comment-translate" aria-label="' + TEXT.translate + '" title="' + TEXT.translate + '">' + iconTranslate() + '</button>' +
          '</div>' +
        '</div>' +
      '</div>';
    }).join('');
  ...
  function setReplyTarget(commentEl) {
    var username = commentEl.dataset.username || '用户';
    var pid = commentEl.dataset.pid || '';
  ...
  function setCommentVoice(blob, duration) {
    ...
    if (!wrap) return;
    wrap.innerHTML = blob
      ? renderVoiceCard({ url: state.comments.voiceUrl }, 'is-preview') + '<button type="button" class="pv-comment-voice-remove">×</button>'
      : '';
  ...
  function toggleCommentRecording() {
    ...
    if (state.comments.mediaRecorder && state.comments.mediaRecorder.state === 'recording') {
      stopCommentRecording(false);
      return;
    }
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      state.comments.stream = stream;
      state.comments.chunks = [];
      state.comments.startAt = Date.now();
      var mime = recorderMime();
      var rec = new MediaRecorder(stream, mime ? { mimeType: mime, audioBitsPerSecond: 16000 } : { audioBitsPerSecond: 16000 });
  ...
  function stopCommentRecording() {
    ...
    if (state.comments.stream) state.comments.stream.getTracks().forEach(function (t) { t.stop(); });
    clearInterval(state.comments.timer);
    var panel = $('.pv-comment-record-panel', state.root);
  ...
  function updateCommentActionButton() {
    ...
    btn.innerHTML = (hasText || hasVoice) ? iconSend() : (recording ? '<span class="pv-stop-dot"></span>' : iconMic());
  }
  function handleCommentAction() {
    var input = $('.pv-comment-input', state.root);
    if ((input && norm(input.value)) || state.comments.voiceBlob) submitComment();
    else toggleCommentRecording();
  }
    ...
    Promise.resolve().then(function () {
      ...
      return uploadToNodeBB(file).then(function (url) {
        return (content ? content + '\n' : '') +
          '[' + TEXT.voiceMsg + ' · ' + formatDuration(state.comments.voiceDuration || 1) + ']' +
          '(' + appendDurationParam(url, state.comments.voiceDuration || 1) + ')';
      });
  ...
  function updateCountUi(item) {
    var slide = findSlide(state.list.indexOf(item));
    if (!slide) return;
    var btn = $('.pv-comment-btn span:last-child', slide);
    if (btn) btn.textContent = formatCount(item.counts.comments);
  }
  ...
  function translateCommentInput() {
    ...
    var text = norm(input.value);
    if (!text) return;
    translateText(text).then(function (out) {
      if (out) { input.value = out; updateCommentActionButton(); }
    }).catch(function () { alertError(TEXT.translateFail); });
  }
  ...
  function setPendingImages(files) {
    ...
    $('.pv-preview-images', state.root).innerHTML = state.compose.imageUrls.map(function (u) {
      return '<img src="' + u + '" alt="preview">';
    }).join('');
  }
  ...
  function buildTitle(text) {
    var clean = norm(String(text || '')
      .replace(RE.tiktokGlobal, '')
      .replace(RE.tiktokShort, '')
      .replace(/!\[[^\]]*\]\([^)]+\)/g, '')
      .replace(/\[[^\]]*\]\([^)]+\)/g, ''));
    return clean ? clean.slice(0, 80) : '动态';
  }
  ...
  function sendTopic() {
    ...
    btn.disabled = true;
    btn.textContent = TEXT.publishing;
    var lines = [];
    ...
    Promise.resolve().then(function () {
      ...
      console.warn(err);
      alertError(err.message || TEXT.publishFail);
    }).finally(function () {
      btn.disabled = false;
  ...
  function translateComposerInput() {
    var ta = $('.pv-compose-panel textarea', state.root);
      ...
      '<div class="pv-page pv-page-active">' +
        ...
        '<section class="pv-compose-panel" role="dialog">' +
          '<div class="pv-panel-head">' +
            ...
            '<button type="button" class="pv-close pv-compose-close">×</button>' +
          '</div>' +
          ...
          '<div class="pv-compose-tools">' +
            ...
            '<button type="button" class="pv-tool pv-compose-translate" aria-label="' + TEXT.translate + '" title="' + TEXT.translate + '">' + iconTranslate() + '</button>' +
        ...
        '<section class="pv-comments-panel" role="dialog">' +
          ...
          '<div class="pv-panel-head pv-comments-drag">' +
            ...
            '<button type="button" class="pv-close pv-comments-close">×</button>' +
          '</div>' +
          ...
          '<div class="pv-comment-record-panel">' +
            '<div class="pv-record-pulse"></div>' +
            '<div class="pv-record-bars"><i></i><i></i><i></i><i></i><i></i></div>' +
            '<span class="pv-record-time">00:00</span>' +
            '<span class="pv-record-tip">正在录音，点右侧按钮停止</span>' +
          '</div>' +
          '<div class="pv-comment-send-row">' +
            '<div class="pv-comment-input-wrap">' +
              '<button type="button" class="pv-comment-input-translate" aria-label="' + TEXT.translate + '" title="' + TEXT.translate + '">' + iconTranslate() + '</button>' +
              '<input class="pv-comment-input" placeholder="' + TEXT.commentPlaceholder + '">' +
              '<button type="button" class="pv-comment-action-btn" aria-label="语音或发送">' + iconMic() + '</button>' +
            '</div>' +
          '</div>' +
        '</section>' +
        ...
        '<section class="pv-translate-panel" role="dialog">' +
          '<div class="pv-panel-head">' +
            ...
            '<button type="button" class="pv-close pv-translate-close">×</button>' +
          '</div>' +
          ...
          '</div>' +
          '<div class="pv-translate-actions">' +
            '<button type="button" class="pv-primary pv-translate-save">' + TEXT.save + '</button>' +
          '</div>' +
        '</section>' +

        '<div class="pv-viewer">' +
          '<div class="pv-viewer-swiper swiper"><div class="swiper-wrapper"></div><div class="pv-viewer-pagination"></div></div>' +
          '<button class="pv-viewer-close" aria-label="关闭">×</button>' +
        '</div>' +
      '</div>';
    bindChrome();
  }
  ...
  function bindLongPress(el, cb) {
    ...
    el.addEventListener('pointerdown', function (e) {
      ...
      clearTimeout(timer);
      timer = setTimeout(function () { cb(e); timer = 0; }, 560);
    });
    el.addEventListener('pointermove', function (e) {
      if (Math.hypot(e.clientX - sx, e.clientY - sy) > 12) { clearTimeout(timer); timer = 0; }
    });
    ['pointerup','pointercancel','pointerleave'].forEach(function (n) {
      el.addEventListener(n, function () { clearTimeout(timer); timer = 0; });
    });
  }
  ...
  function bindChrome() {
    ...
    $('.pv-compose-translate', state.root).addEventListener('click', translateComposerInput);
    ...
    $('.pv-comment-voice-preview', state.root).addEventListener('click', function (e) {
      ...
      var card = e.target.closest('.pv-voice-card');
      if (card) toggleVoiceCard(card);
    });
    ...
    viewer.addEventListener('pointerdown', function (e) {
      state.viewer.down = true; state.viewer.startX = e.clientX; state.viewer.startY = e.clientY;
    });
    viewer.addEventListener('pointerup', function (e) {
      if (!state.viewer.down) return;
      state.viewer.down = false;
      var dx = e.clientX - state.viewer.startX;
      var dy = e.clientY - state.viewer.startY;
      if (dy > 66 && Math.abs(dy) > Math.abs(dx)) closeViewer();
    });

    document.addEventListener('visibilitychange', function () {
      if (document.hidden) pauseSlide(state.index); else activateCurrent(false);
    });
  }
  ...
  function onRootClick(e) {
    ...
    if ((btn = e.target.closest('.pv-like'))) {
      e.preventDefault(); e.stopPropagation();
      var i = Number(btn.dataset.index);
      toggleLike(state.list[i], findSlide(i), true);
      return;
    }
    if ((btn = e.target.closest('.pv-comment-btn'))) {
      e.preventDefault(); e.stopPropagation();
      openComments(state.list[Number(btn.dataset.index)]);
      return;
    }
    if ((btn = e.target.closest('.pv-follow-plus'))) {
      e.preventDefault(); e.stopPropagation();
      var idx = Number(btn.dataset.index);
      toggleFollow(state.list[idx], findSlide(idx));
      return;
    }
    if ((btn = e.target.closest('.pv-translate-btn'))) {
      e.preventDefault(); e.stopPropagation();
      var ti = Number(btn.dataset.index);
      translateSlide(state.list[ti], findSlide(ti));
      return;
    }
    if ((btn = e.target.closest('.pv-album-btn'))) {
      e.preventDefault(); e.stopPropagation();
      var pi = Number(btn.dataset.index);
      openViewer(state.list[pi].images || [], 0);
      return;
    }
    if ((btn = e.target.closest('.pv-voice-card'))) {
      e.preventDefault(); e.stopPropagation();
      toggleVoiceCard(btn);
      return;
    }
    if ((btn = e.target.closest('.pv-comment-reply'))) {
      e.preventDefault(); e.stopPropagation();
      setReplyTarget(e.target.closest('.pv-comment'));
      return;
    }
    if ((btn = e.target.closest('.pv-comment-translate'))) {
      e.preventDefault(); e.stopPropagation();
      translateComment(e.target.closest('.pv-comment'));
      return;
    }
  ...
  function onRootPointerDown(e) {
    ...
    var translateBtn = e.target.closest('.pv-translate-btn, .pv-comment-translate, .pv-comment-input-translate, .pv-compose-translate');
    ...
    if (panel && panel.classList.contains('is-open') && e.target.closest('.pv-comments-panel') && !e.target.closest('input,button,.pv-voice-card')) {
      ...
      state.comments.dragStartTopZone = true;
  ...
  function onRootPointerCancel() {
    ...
    state.comments.dragging = false;
    state.comments.dragCandidate = false;
    var p = $('.pv-comments-panel', state.root);
    if (p) p.style.transform = '';
  }

  function showEmpty(text) {
    var old = $('.pv-empty-page', state.root);
    if (old) old.remove();
    var div = document.createElement('div');
    div.className = 'pv-empty-page';
    div.textContent = text || TEXT.empty;
